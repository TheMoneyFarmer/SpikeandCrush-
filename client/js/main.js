...
(function () {
  ...
  TW.requireAuth = () => {
    if (!TW.isLoggedIn()) {
      ...
      // '/' (landing.html) is the one logged-out entry point for the whole
      // app - it has its own login/register slide-out panel and auto-opens
      // it when it sees ?login=required (see landing.html's own init).
  ...
  function initIndexPage() {
    ...
    // /play (this page) is hub-only now - landing.html at '/' is the single
    // logged-out entry point for everyone (it has its own login/register
    // slide-out panel), so a logged-out visitor here gets bounced there
    ...
    // /play?join=123456 - the not-logged-in case already redirected away
    // above (preserving this exact URL as the post-login redirect), so
    // reaching here means we're logged in and can join right away.
    const joinCode = new URLSearchParams(window.location.search).get('join');
    if (joinCode) joinByRoomCode(joinCode);
